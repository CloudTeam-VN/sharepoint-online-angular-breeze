...
(function (definition) {
  ...
  function _createChangeRequest(saveContext, entity, index) {
    ...
    if (state.isAdded()) {
      ...
      data = adapter._serializeToJson(processComplexData(rawEntity, type));
      ...
      var data = processComplexData(rawEntity, type);
      request.data = adapter._serializeToJson(data);
  ...
  // Process Complex Data - NDQUOC
  function processComplexData(rawEntity, type) {
    var data = rawEntity;
    type.dataProperties.forEach(function (prop) {
      var objData = data[prop.name];
      var _dataType = prop.dataType;
      var customAttr = _dataType.custom || {};
      if (prop.dataType.isComplexType) {
        if (objData) {
          if (customAttr.isLookup === true) {
            if (Array.isArray(objData)) {
              var updateValue = [];
              objData.forEach(function (val) {
                updateValue.push(val.Id);
              });
              data[prop.name + 'Id'] = { results: updateValue };
            } else {
              data[prop.name + 'Id'] = objData.Id
            }
            delete data[prop.name];
          } else {
            if (customAttr['__metadata']) {
              objData['__metadata'] = customAttr['__metadata'];
            }
          }
        } else {
          delete data[prop.name];
  ...
  function _createJsonResultsAdapter() {
    ...
    function visitNode(node, mappingContext, nodeContext) {
      ...
      // fix issue missing extraMetadata
  ...
  function executeQuery(mappingContext) {
    ...
    mappingContext.entityType.dataProperties.forEach(function (prop) {
      var _dataType = prop.dataType;
      var customAttr = _dataType.custom || {};
      if (_dataType.isComplexType === true && customAttr.isLookup === true) {
        expand.push(prop);
      }
    });

    if (expand.length) {
      angular.forEach(expand, function (extraType) {
        extraType.dataType.dataProperties.forEach(function (prop) {
          select.push(extraType.name + '/' + prop.name);
        });
        expandProperties.push(extraType.name);
      });
    }
